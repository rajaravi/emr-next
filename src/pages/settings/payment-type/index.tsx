...
import { PaymentTypeFormElements } from '@/data/PaymentTypeFormElements';
import { PaymentTypeModel } from '@/types/payment-type';
...
const PaymentType: React.FC = () => {
  ...
  const columns: { name: string; class: string; field: string; }[] = [
    { name: t('SETTING.PAYMENT_TYPE.SNO'), class: "col-sm-1", field: "sno"},
    { name: t('SETTING.PAYMENT_TYPE.NAME'), class: "col-sm-5", field: "name"},
    { name: t('SETTING.PAYMENT_TYPE.DEFAULT'), class: "col-sm-3", field: "is_default"},
    { name: t('SETTING.PAYMENT_TYPE.ARCHIVE'), class: "col-sm-3", field: "is_archive"}
  ];
  const filter: { name: string; field: string; }[] = [
    { name: t('SETTING.PAYMENT_TYPE.NAME'), field: 'name' }
  ];

  const dynamicFormRef = useRef<DynamicFormHandle>(null);
  const [page, setPage] = useState<number>(1);
  const [total, setTotal] = useState<number>(0);
  const [selectedIncomeCategory, setSelectedPaymentType] = useState<number>(0);
  ...
  const initialFormData: PaymentTypeModel = {
    "id": null,
    "type_id": 1,
    "name": "",
    "is_archive": false,
  ...
  const [formData, setFormData] = useState<PaymentTypeModel>(initialFormData);
  const handleShow = () => {
    setShow(true);
  ...
  useEffect(() => {    
    ...
    const translatedFormElements = PaymentTypeFormElements.map((element) => ({
      ...element,
      label: t('SETTING.PAYMENT_TYPE.'+element.label)
    }));
    setTranslatedElements(translatedFormElements);
    fetchPaymentTypeList(page);
  }, []);

  // Get doctor list
  const fetchPaymentTypeList = async (page: number, sFilter?: { field: string; text: string }) => {
    showLoading();
    try {
      let passData: string = JSON.stringify({ page: page, limit: pageLimit, sort: null, search: sFilter, type_id: 2 });
      const response = await execute_axios_post(ENDPOINTS.POST_CATEGORY_LIST, passData);
  ...
  const handleSearch = () => {
    ...
    if (searchTextElement.value) {
        ...
        fetchPaymentTypeList(1,sFilter);
        setClear(true);
    }
  }
  ...
  const clearSearch = () => {
    ...
    setsearchFilter([]);
    fetchPaymentTypeList(1);
    setClear(false);
  }

  // List double click
  const paymentTypeDblClick = (event: any) => {
    ...
    getPaymentTypeById('edit');
  }

  // List single click
  const paymentTypeClick = (event: any) => {
    let x = document.getElementsByClassName("selected");
    if(x.length > 0) { x[0].classList.remove("selected"); }
  ...
  // Get form data
  const getPaymentTypeById = async (type: string) => {
    try {
      let editID = 0;      
      if(type == 'edit') editID = selectedIncomeCategory;
      let passData: string = JSON.stringify({ id: editID });      
      const response = await execute_axios_post(ENDPOINTS.POST_CATEGORY_FORMDATA, passData);
      if(response.success) {        
        handleShow();
  ...
  const handleSave = async () => {
    ...
    if (dynamicFormRef.current?.validateModelForm()) {
      try {
        formData.type_id = 2;
        const response = await execute_axios_post(ENDPOINTS.POST_CATEGORY_STORE, formData);
        handleShowToast(t('SETTING.PAYMENT_TYPE.MESSAGES.SAVE_SUCCESS'), 'success');
      } catch (error) {
  ...
  const handleArchive = async(event: any) => {
    ...
    try {
      ...
      const response = await execute_axios_post(ENDPOINTS.POST_CATEGORY_ARCHIVE, passData);      
      if(response.success) { 
  ...
  const handleDefault = async(event: any) => {
    ...
    try {
      ...
      const response = await execute_axios_post(ENDPOINTS.POST_CATEGORY_DEFAULT, passData);      
      if(response.success) { 
        if(event.target.checked === true) {
  ...
  const refreshData = (currentPage: number) => {
    ...
    setPage(currentPage);
    fetchPaymentTypeList(currentPage, searchFilter);
  }
  ...
  return (
    <SettingLayout>
      <div className="d-flex justify-content-between align-items-center">
        <h1 className={`${styles.title} mb-3 module-title`}><i className="fi fi-rr-wallet-arrow"></i> {t('SETTING.SIDE_MENU.PAYMENT_TYPE')}</h1>
      </div>
      <Row className="white-bg p-1 m-0 top-bottom-shadow">
        <Col xs={7} className="mt-3 action">
          <Button variant='primary' className='btn rounded-0' onClick={createPaymentType}><i className="fi fi-ss-add"></i> {t('ACTIONS.ADDNEW')}</Button>
          <Dropdown >
      ...
      <div>
        <Datalist
          columns={columns}
          list={list}
          onRowDblClick={paymentTypeDblClick}
          onRowClick={paymentTypeClick}
          page={page}
          total={total}
          pageLimit={pageLimit}
      ...
      <OffcanvasComponent
        show={show}
        title={ (mode) ? t('SETTING.PAYMENT_TYPE.EDIT_TITLE') : t('SETTING.PAYMENT_TYPE.CREATE_TITLE') }
