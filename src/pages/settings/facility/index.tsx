...
import { FacilityFormElements } from '@/data/FacilityFormElements';
import { FacilityModel } from '@/types/facility';
...
const Facility: React.FC = () => {
  ...
  const columns: { name: string; class: string; field: string; }[] = [
    { name: t('SETTING.FACILITY.SNO'), class: "col-sm-1", field: "sno"},
    { name: t('SETTING.FACILITY.NAME'), class: "col-sm-4", field: "name"},
    { name: t('SETTING.FACILITY.CONTACT_NAME'), class: "col-sm-3", field: "contact_name"},    
    { name: t('SETTING.FACILITY.MOBILE_NO'), class: "col-sm-2", field: "mobile_no"},
    { name: t('SETTING.FACILITY.ARCHIVE'), class: "col-sm-2", field: "is_archive"}
  ];
  const filter: { name: string; field: string; }[] = [
    { name: t('SETTING.FACILITY.NAME'), field: 'name' }    
  ];
  ...
  const initialFormData: FacilityModel = {
    "id": null,
    "name": "",
    "contact_name": "",
    "address1": "",
  ...
  const [formData, setFormData] = useState<FacilityModel>(initialFormData);
  const handleShow = () => {
    setShow(true);
  ...
  useEffect(() => {    
    ...
    const translatedFormElements = FacilityFormElements.map((element) => ({
      ...element,
      label: t('SETTING.FACILITY.'+element.label)
    }));
    setTranslatedElements(translatedFormElements);
    fetchFacilityList(page);
  }, []);

  // Get doctor list
  const fetchFacilityList = async (page: number, sFilter?: { field: string; text: string }) => {
    showLoading();
    try {
      let passData: string = JSON.stringify({ page: page, limit: pageLimit, sort: null, search: sFilter });
      const response = await execute_axios_post(ENDPOINTS.POST_FACILITY_LIST, passData);
  ...
  // List double click
  const facilityDblClick = (event: any) => {
    ...
    getFacilityById('edit');
  }

  // List single click
  const facilityClick = (event: any) => {
    let x = document.getElementsByClassName("selected");
    if(x.length > 0) { x[0].classList.remove("selected"); }
  ...
  // Get form data
  const getFacilityById = async (type: string) => {
    try {
      let editID = 0;      
      if(type == 'edit') editID = selectedFacility;
      let passData: string = JSON.stringify({ id: editID });      
      const response = await execute_axios_post(ENDPOINTS.POST_FACILITY_FORMDATA, passData);
      if(response.success) {        
        handleShow();
        ...
        console.error('Error on fetching facility details:', error);
    }
  }
  ...
  const handleSave = async () => {
    ...
    if (dynamicFormRef.current?.validateModelForm()) {
      try {
        const response = await execute_axios_post(ENDPOINTS.POST_FACILITY_STORE, formData);
        handleShowToast(t('SETTING.FACILITY.MESSAGES.SAVE_SUCCESS'), 'success');
      } catch (error) {
  ...
  const handleArchive = async(event: any) => {
    ...
    try {
      ...
      const response = await execute_axios_post(ENDPOINTS.POST_FACILITY_ARCHIVE, passData);      
      if(response.success) { 
  ...
  const refreshData = (currentPage: number) => {
    ...
    setPage(currentPage);
    fetchFacilityList(currentPage, searchFilter);
  }
  ...
  return (
    <SettingLayout>
      <div className="d-flex justify-content-between align-items-center">
        <h1 className={`${styles.title} mb-3 module-title`}><i className="fi fi-rr-marker"></i> {t('SETTING.SIDE_MENU.FACILITY')}</h1>
      </div>
      <Row className="white-bg p-1 m-0 top-bottom-shadow">
        <Col xs={7} className="mt-3 action">
          <Button variant='primary' className='btn rounded-0' onClick={createFacility}><i className="fi fi-ss-add"></i> {t('ACTIONS.ADDNEW')}</Button>
          <Dropdown >
      ...
      <div>
        <Datalist
          columns={columns}
          list={list}
          onRowDblClick={facilityDblClick}
          onRowClick={facilityClick}
          page={page}
          total={total}
          pageLimit={pageLimit}
      ...
      <OffcanvasComponent
        show={show}
        title={ (mode) ? t('SETTING.FACILITY.EDIT_TITLE') : t('SETTING.FACILITY.CREATE_TITLE') }
