...
import { PurchaserTypeFormElements } from '@/data/PurchaserTypeFormElements';
import { PurchaserTypeModel } from '@/types/purchaser-type';
...
const PurchaserType: React.FC = () => {
  const { showLoading, hideLoading } = useLoading();
  const [show, setShow] = useState(false);
  const { t } = useTranslation('common');
  const columns: { name: string; class: string; field: string; }[] = [
    { name: t('SETTING.PURCHASER_TYPE.SNO'), class: "col-sm-1", field: "sno"},
    { name: t('SETTING.PURCHASER_TYPE.NAME'), class: "col-sm-8", field: "name"},
    { name: t('SETTING.PURCHASER_TYPE.ARCHIVE'), class: "col-sm-3", field: "is_archive"}
  ];
  const filter: { name: string; field: string; }[] = [
    { name: t('SETTING.PURCHASER_TYPE.NAME'), field: 'name' }
  ];

  const dynamicFormRef = useRef<DynamicFormHandle>(null);
  const [page, setPage] = useState<number>(1);
  const [total, setTotal] = useState<number>(0);
  const [selectedPurchaserType, setSelectedPurchaserType] = useState<number>(0);
  ...
  const initialFormData: PurchaserTypeModel = {
    "id": null,
    "name": "",
    "is_archive": false
  };
  const [formData, setFormData] = useState<PurchaserTypeModel>(initialFormData);
  const handleShow = () => {
    setShow(true);
  ...
  useEffect(() => {    
    ...
    const translatedFormElements = PurchaserTypeFormElements.map((element) => ({
      ...element,
      label: t('SETTING.PURCHASER_TYPE.'+element.label)
    }));
    setTranslatedElements(translatedFormElements);
    fetchPurchaserTypeList(page);
  }, []);

  // Get doctor list
  const fetchPurchaserTypeList = async (page: number, sFilter?: { field: string; text: string }) => {
    showLoading();
    try {
      let passData: string = JSON.stringify({ page: page, limit: pageLimit, sort: null, search: sFilter });
      const response = await execute_axios_post(ENDPOINTS.POST_PURCHASER_TYPE_LIST, passData);
      setList(response.data.list);
      setTotal(response.data.total);
    } catch (err) {
      setError('Failed to load purchaser type category data.');
    } finally {
      hideLoading();
    }
  }; 
  ...
  const handleSearch = () => {
    ...
    if (searchTextElement.value) {
        ...
        fetchPurchaserTypeList(1,sFilter);
        setClear(true);
    }
  }
  ...
  const clearSearch = () => {
    ...
    setsearchFilter([]);
    fetchPurchaserTypeList(1);
    setClear(false);
  }

  // List double click
  const purchaserTypeDblClick = (event: any) => {
    ...
    if(event.target.parentNode.getAttribute('custom-id')) {
      ...
      setSelectedPurchaserType(selectedID);
    }
    getPurchaserTypeById('edit');
  }

  // List single click
  const purchaserTypeClick = (event: any) => {
    let x = document.getElementsByClassName("selected");
    if(x.length > 0) { x[0].classList.remove("selected"); }
  ...
  // Edit action call
  const createPurchaserType = () => {    
    getPurchaserTypeById('add');
  }
  
  // Edit action call
  const handleEdit = () => {
    if(selectedPurchaserType === 0) {
  ...
  // Get form data
  const getPurchaserTypeById = async (type: string) => {
    try {
      let editID = 0;      
      if(type == 'edit') editID = selectedPurchaserType;
      let passData: string = JSON.stringify({ id: editID });
      const response = await execute_axios_post(ENDPOINTS.POST_PURCHASER_TYPE_FORMDATA, passData);
      if(response.success) {        
        handleShow();
        ...
        console.error('Error on fetching purchaser type details:', error);
    }
  }
  ...
  const handleSave = async () => {
    ...
    if (dynamicFormRef.current?.validateModelForm()) {
      try {
        const response = await execute_axios_post(ENDPOINTS.POST_PURCHASER_TYPE_STORE, formData);
        handleShowToast(t('SETTING.PURCHASER_TYPE.MESSAGES.SAVE_SUCCESS'), 'success');
      } catch (error) {
  ...
  const handleArchive = async(event: any) => {
    ...
    try {
      ...
      const response = await execute_axios_post(ENDPOINTS.POST_PURCHASER_TYPE_ARCHIVE, passData);      
      if(response.success) { 
  ...
  const refreshData = (currentPage: number) => {
    ...
    setSelectedPurchaserType(0);
    setPage(currentPage);
    fetchPurchaserTypeList(currentPage, searchFilter);
  }
  ...
  return (
    <SettingLayout>
      <div className="d-flex justify-content-between align-items-center">
        <h1 className={`${styles.title} mb-3`}>{t('SETTING.SIDE_MENU.PURCHASER_TYPE')}</h1>
      </div>
      <Row className="white-bg p-1 m-0 top-bottom-shadow">
        <Col xs={7} className="mt-3 action">
          <Button variant='primary' className='btn rounded-0' onClick={createPurchaserType}><i className="fi fi-ss-add"></i> {t('ACTIONS.ADDNEW')}</Button>
          <Dropdown >
      ...
      <div>
        <Datalist
          columns={columns}
          list={list}
          onRowDblClick={purchaserTypeDblClick}
          onRowClick={purchaserTypeClick}
          page={page}
          total={total}
          pageLimit={pageLimit}
      ...
      <OffcanvasComponent
        show={show}
        title={ (mode) ? t('SETTING.PURCHASER_TYPE.EDIT_TITLE') : t('SETTING.PURCHASER_TYPE.CREATE_TITLE') }
