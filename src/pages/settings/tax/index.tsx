...
import { TaxFormElements } from '@/data/TaxFormElements';
import { TaxModel } from '@/types/tax';
...
const Tax: React.FC = () => {
  ...
  const columns: { name: string; class: string; field: string; }[] = [
    { name: t('SETTING.TAX.SNO'), class: "col-sm-1", field: "sno"},
    { name: t('SETTING.TAX.NAME'), class: "col-sm-5", field: "name"},
    { name: t('SETTING.TAX.DEFAULT'), class: "col-sm-3", field: "is_default"},
    { name: t('SETTING.TAX.ARCHIVE'), class: "col-sm-3", field: "is_archive"}
  ];
  const filter: { name: string; field: string; }[] = [
    { name: t('SETTING.TAX.NAME'), field: 'name' }
  ];

  const dynamicFormRef = useRef<DynamicFormHandle>(null);
  const [page, setPage] = useState<number>(1);
  const [total, setTotal] = useState<number>(0);
  const [selectedIncomeCategory, setSelectedTax] = useState<number>(0);
  ...
  const initialFormData: TaxModel = {
    "id": null,    
    "name": "",
    "percentage": "",
    "is_archive": false,
  ...
  const [formData, setFormData] = useState<TaxModel>(initialFormData);
  const handleShow = () => {
    setShow(true);
  ...
  useEffect(() => {    
    ...
    const translatedFormElements = TaxFormElements.map((element) => ({
      ...element,
      label: t('SETTING.TAX.'+element.label)
    }));
  ...
  // Get doctor list
  const fetchTaxList = async (page: number, sFilter?: { field: string; text: string }) => {
    showLoading();
    try {
      let passData: string = JSON.stringify({ page: page, limit: pageLimit, sort: null, search: sFilter });
      const response = await execute_axios_post(ENDPOINTS.POST_TAX_LIST, passData);
  ...
  const taxDblClick = (event: any) => {
    ...
    if(event.target.parentNode.getAttribute('custom-id')) {
      ...
      setSelectedTax(selectedID);
    }
    getTaxById('edit');
  }

  // List single click
  const taxClick = (event: any) => {
    let x = document.getElementsByClassName("selected");
    if(x.length > 0) { x[0].classList.remove("selected"); }
  ...
  // Get form data
  const getTaxById = async (type: string) => {
    try {
      let editID = 0;      
      if(type == 'edit') editID = selectedIncomeCategory;
      let passData: string = JSON.stringify({ id: editID });      
      const response = await execute_axios_post(ENDPOINTS.POST_TAX_FORMDATA, passData);
      if(response.success) {        
        handleShow();
        ...
        console.error('Error on fetching tax details:', error);
    }
  }
  ...
  const handleSave = async () => {
    ...
    if (dynamicFormRef.current?.validateModelForm()) {
      try {
        const response = await execute_axios_post(ENDPOINTS.POST_TAX_STORE, formData);
        handleShowToast(t('SETTING.TAX.MESSAGES.SAVE_SUCCESS'), 'success');
      } catch (error) {
  ...
  const handleArchive = async(event: any) => {
    ...
    try {
      ...
      const response = await execute_axios_post(ENDPOINTS.POST_TAX_ARCHIVE, passData);      
      if(response.success) { 
  ...
  const refreshData = (currentPage: number) => {
    ...
    setSelectedTax(0);
    setPage(currentPage);
    fetchTaxList(currentPage, searchFilter);
  }
  ...
  return (
    <SettingLayout>
      <div className="d-flex justify-content-between align-items-center">
        <h1 className={`${styles.title} mb-3 module-title`}><i className="fi fi-rr-calculator-math-tax"></i> {t('SETTING.SIDE_MENU.TAX')}</h1>
      </div>
      <Row className="white-bg p-1 m-0 top-bottom-shadow">
        <Col xs={7} className="mt-3 action">
          <Button variant='primary' className='btn rounded-0' onClick={createTax}><i className="fi fi-ss-add"></i> {t('ACTIONS.ADDNEW')}</Button>
          <Dropdown >
      ...
      <OffcanvasComponent
        show={show}
        title={ (mode) ? t('SETTING.TAX.EDIT_TITLE') : t('SETTING.TAX.CREATE_TITLE') }
