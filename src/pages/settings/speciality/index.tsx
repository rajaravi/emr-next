...
import { SpecialityFormElements } from '@/data/SpecialityFormElements';
import { SpecialityModel } from '@/types/speciality';
...
const Speciality: React.FC = () => {
  ...
  const columns: { name: string; class: string; field: string; }[] = [
    { name: t('SETTING.SPECIALITY.SNO'), class: "col-sm-1", field: "sno"},
    { name: t('SETTING.SPECIALITY.NAME'), class: "col-sm-8", field: "name"},
    { name: t('SETTING.SPECIALITY.ARCHIVE'), class: "col-sm-3", field: "is_archive"}
  ];
  const filter: { name: string; field: string; }[] = [
    { name: t('SETTING.SPECIALITY.NAME'), field: 'name' }
  ];
  ...
  const initialFormData: SpecialityModel = {
    "id": null,
    "name": "",
    "is_archive": false
  };
  const [formData, setFormData] = useState<SpecialityModel>(initialFormData);
  const handleShow = () => {
    setShow(true);
  ...
  useEffect(() => {    
    ...
    const translatedFormElements = SpecialityFormElements.map((element) => ({
      ...element,
      label: t('SETTING.SPECIALITY.'+element.label)
    }));
    setTranslatedElements(translatedFormElements);
    fetchSpecialityList(page);
  }, []);

  // Get doctor list
  const fetchSpecialityList = async (page: number, sFilter?: { field: string; text: string }) => {
    showLoading();
    try {
      let passData: string = JSON.stringify({ page: page, limit: pageLimit, sort: null, search: sFilter });
      const response = await execute_axios_post(ENDPOINTS.POST_SPECIALITY_LIST, passData);
      setList(response.data.list);
      setTotal(response.data.total);
    } catch (err) {
      setError('Failed to load speciality data.');
    } finally {
      hideLoading();
    }
  }; 
  ...
  const clearSearch = () => {
    ...
    setsearchFilter([]);
    fetchSpecialityList(1);
    setClear(false);
  }

  // List double click
  const specialityDblClick = (event: any) => {
    ...
    getSpecialityById('edit');
  }

  // List single click
  const specialityClick = (event: any) => {
    let x = document.getElementsByClassName("selected");
    if(x.length > 0) { x[0].classList.remove("selected"); }
  ...
  const createSpeciality = () => {
    getSpecialityById('add');
  }
  
  // Edit action call
  const handleEdit = () => {
    if(selectedSpeciality === 0) {
  ...
  // Get form data
  const getSpecialityById = async (type: string) => {
    try {
      let editID = 0;      
      if(type == 'edit') editID = selectedSpeciality;
      let passData: string = JSON.stringify({ id: editID });      
      const response = await execute_axios_post(ENDPOINTS.POST_SPECIALITY_FORMDATA, passData);
      if(response.success) {        
        handleShow();
        ...
        console.error('Error on fetching speciality details:', error);
    }
  }
  ...
  const handleSave = async () => {
    ...
    if (dynamicFormRef.current?.validateModelForm()) {
      try {
        const response = await execute_axios_post(ENDPOINTS.POST_SPECIALITY_STORE, formData);
        handleShowToast(t('SETTING.SPECIALITY.MESSAGES.SAVE_SUCCESS'), 'success');
      } catch (error) {
  ...
  const handleArchive = async(event: any) => {
    ...
    try {
      ...
      const response = await execute_axios_post(ENDPOINTS.POST_SPECIALITY_ARCHIVE, passData);      
      if(response.success) { 
  ...
  const refreshData = (currentPage: number) => {
    ...
    setPage(currentPage);
    fetchSpecialityList(currentPage, searchFilter);
  }
  ...
  return (
    <SettingLayout>
      <div className="d-flex justify-content-between align-items-center">
        <h1 className={`${styles.title} mb-3 module-title`}><i className="fi fi-rr-member-list"></i> {t('SETTING.SIDE_MENU.SPECIALITY')}</h1>
      </div>
      ...
      <div>
        <Datalist
          columns={columns}
          list={list}
          onRowDblClick={specialityDblClick}
          onRowClick={specialityClick}
          page={page}
          total={total}
          pageLimit={pageLimit}
      ...
      <OffcanvasComponent
        show={show}
        title={ (mode) ? t('SETTING.SPECIALITY.EDIT_TITLE') : t('SETTING.SPECIALITY.CREATE_TITLE') }
