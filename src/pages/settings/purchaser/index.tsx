...
import { PurchaserFormElements } from '@/data/PurchaserFormElements';
import { PurchaserModel } from '@/types/purchaser';
...
const Purchaser: React.FC = () => {
  ...
  const columns: { name: string; class: string; field: string; }[] = [
    { name: t('SETTING.PURCHASER.SNO'), class: "col-sm-1", field: "sno"},
    { name: t('SETTING.PURCHASER.NAME'), class: "col-sm-8", field: "name"},
    { name: t('SETTING.PURCHASER.ARCHIVE'), class: "col-sm-3", field: "is_archive"}
  ];
  const filter: { name: string; field: string; }[] = [
    { name: t('SETTING.PURCHASER.NAME'), field: 'name' }
  ];

  const dynamicFormRef = useRef<DynamicFormHandle>(null);
  const [page, setPage] = useState<number>(1);
  const [total, setTotal] = useState<number>(0);
  const [selectedPurchaser, setSelectedProcedure] = useState<number>(0);
  const [mode, setMode] = useState<boolean>(false);
  const [clear, setClear] = useState<boolean>(false);
  const [list, setList] = useState<any>([]);
  const [searchFilter, setsearchFilter] = useState<any>([]);
  const [initialValues, setInitialValues] = useState<any>(initialValue);  
  const [purchaserTypeList, setProcedureTypeList] = useState<any>();
  ...
  const [formData, setFormData] = useState<PurchaserModel>(initialFormData);
  const handleShow = () => {
    setShow(true);
  ...
  useEffect(() => {    
    ...
    const translatedFormElements = PurchaserFormElements.map((element) => ({
      ...element,
      label: t('SETTING.PURCHASER.'+element.label)
    }));
    setTranslatedElements(translatedFormElements);
    setPurchaserList(page);
  }, []);

  // Get doctor list
  const setPurchaserList = async (page: number, sFilter?: { field: string; text: string }) => {
    showLoading();
    try {
      let passData: string = JSON.stringify({ page: page, limit: pageLimit, sort: null, search: sFilter });
      const response = await execute_axios_post(ENDPOINTS.POST_PURCHASER_LIST, passData);
  ...
  // List double click
  const purchaserDblClick = (event: any) => {
    ...
    getPurchaserById('edit');
  }

  // List single click
  const purchaserClick = (event: any) => {
    let x = document.getElementsByClassName("selected");
    if(x.length > 0) { x[0].classList.remove("selected"); }
  ...
  const createPurchaser = () => {    
    getPurchaserById('add');
  }
  
  // Edit action call
  const handleEdit = () => {
    if(selectedPurchaser === 0) {
  ...
  // Get form data
  const getPurchaserById = async (type: string) => {
    try {
      let editID = 0;      
      if(type == 'edit') editID = selectedPurchaser;
      let passData: string = JSON.stringify({ id: editID });    
      const response = await execute_axios_post(ENDPOINTS.POST_PURCHASER_FORMDATA, passData);
      if(response.success) {        
        handleShow();
  ...
  const handleSave = async () => {
    ...
    if (dynamicFormRef.current?.validateModelForm()) {
      try {
        const response = await execute_axios_post(ENDPOINTS.POST_PURCHASER_STORE, formData);
        if(response.success) {
          handleShowToast(t('SETTING.PURCHASER.MESSAGES.SAVE_SUCCESS'), 'success');
        }
      } catch (error: any) {
  ...
  const handleArchive = async(event: any) => {
    ...
    try {
      ...
      const response = await execute_axios_post(ENDPOINTS.POST_PURCHASER_ARCHIVE, passData);      
      if(response.success) { 
        ...
        console.error('Error on fetching purchaser details:', error);
        hideLoading();
    }
  }
  ...
  const refreshData = (currentPage: number) => {
    ...
    setPage(currentPage);
    setPurchaserList(currentPage, searchFilter);
  }
  ...
  return (
    <SettingLayout>
      <div className="d-flex justify-content-between align-items-center">
        <h1 className={`${styles.title} mb-3 module-title`}><i className="fi fi-rr-compliance-document"></i> {t('SETTING.SIDE_MENU.PURCHASER')}</h1>
      </div>
      <Row className="white-bg p-1 m-0 top-bottom-shadow">
        <Col xs={7} className="mt-3 action">
          <Button variant='primary' className='btn rounded-0' onClick={createPurchaser}><i className="fi fi-ss-add"></i> {t('ACTIONS.ADDNEW')}</Button>
          <Dropdown >
      ...
      <div>
        <Datalist
          columns={columns}
          list={list}
          onRowDblClick={purchaserDblClick}
          onRowClick={purchaserClick}
          page={page}
          total={total}
          pageLimit={pageLimit}
      ...
      <OffcanvasComponent
        show={show}
        title={ (mode) ? t('SETTING.PURCHASER.EDIT_TITLE') : t('SETTING.PURCHASER.CREATE_TITLE') }
