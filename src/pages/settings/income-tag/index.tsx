...
const IncomeCategory: React.FC = () => {
  ...
  const columns: { name: string; class: string; field: string; }[] = [
    { name: t('SETTING.INCOME_TAG.SNO'), class: "col-sm-1", field: "sno"},
    { name: t('SETTING.INCOME_TAG.NAME'), class: "col-sm-5", field: "name"},
    { name: t('SETTING.INCOME_TAG.DEFAULT'), class: "col-sm-3", field: "is_default"},
    { name: t('SETTING.INCOME_TAG.ARCHIVE'), class: "col-sm-3", field: "is_archive"}
  ];
  const filter: { name: string; field: string; }[] = [
    { name: t('SETTING.INCOME_TAG.NAME'), field: 'name' }
  ];
  ...
  useEffect(() => {    
    ...
    const translatedFormElements = IncomeCategoryFormElements.map((element) => ({
      ...element,
      label: t('SETTING.INCOME_TAG.'+element.label)
    }));
    setTranslatedElements(translatedFormElements);
    fetchIncomeTagList(page);
  }, []);

  // Get doctor list
  const fetchIncomeTagList = async (page: number, sFilter?: { field: string; text: string }) => {
    showLoading();
    try {
      let passData: string = JSON.stringify({ page: page, limit: pageLimit, sort: null, search: sFilter, type_id: 1 });
  ...
  const clearSearch = () => {
    ...
    setsearchFilter([]);
    fetchIncomeTagList(1);
    setClear(false);
  }

  // List double click
  const incomeTagDblClick = (event: any) => {
    ...
    getIncomeTagById('edit');
  }

  // List single click
  const incomeTagClick = (event: any) => {
    let x = document.getElementsByClassName("selected");
    if(x.length > 0) { x[0].classList.remove("selected"); }
  ...
  // Edit action call
  const createIncomeTag = () => {
    getIncomeTagById('add');
  }
  ...
  // Get form data
  const getIncomeTagById = async (type: string) => {
    try {
      let editID = 0;      
      ...
      if(response.success) {        
        ...
        if(response.data?.data?.id) {
          ...
          //   isDefault = 1;
          // }
          // const updatedFormData = { ...formData, is_default: isDefault };
  ...
  const handleSave = async () => {
    ...
    if (dynamicFormRef.current?.validateModelForm()) {
      try {
        ...
        handleShowToast(t('SETTING.INCOME_TAG.MESSAGES.SAVE_SUCCESS'), 'success');
      } catch (error) {
  ...
  const refreshData = (currentPage: number) => {
    ...
    setPage(currentPage);
    fetchIncomeTagList(currentPage, searchFilter);
  }
  ...
  return (
    <SettingLayout>
      <div className="d-flex justify-content-between align-items-center">
        <h1 className={`${styles.title} mb-3 module-title`}><i className="fi fi-rs-category-alt"></i> {t('SETTING.SIDE_MENU.INCOME_TAG')}</h1>
      </div>
      <Row className="white-bg p-1 m-0 top-bottom-shadow">
        <Col xs={7} className="mt-3 action">
          <Button variant='primary' className='btn rounded-0' onClick={createIncomeTag}><i className="fi fi-ss-add"></i> {t('ACTIONS.ADDNEW')}</Button>
          <Dropdown >
      ...
      <div>
        <Datalist
          columns={columns}
          list={list}
          onRowDblClick={incomeTagDblClick}
          onRowClick={incomeTagClick}
          page={page}
          total={total}
          pageLimit={pageLimit}
      ...
      <OffcanvasComponent
        show={show}
        title={ (mode) ? t('SETTING.INCOME_TAG.EDIT_TITLE') : t('SETTING.INCOME_TAG.CREATE_TITLE') }
